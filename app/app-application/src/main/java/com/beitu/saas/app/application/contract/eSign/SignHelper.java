package com.beitu.saas.app.application.contract.eSign;

import com.beitu.saas.app.application.contract.utils.FileHelper;
...
public class SignHelper {
    ...
    private static Log LOGGER = LogFactory.getLog(SignHelper.class);
    
    /***
     * 项目初始化 使用到的接口：sdk.init(proCfg, httpConCfg, sCfg);
     */
    public static void initProject(String projectId, String projectSecret, String apisUrl) {
        ProjectConfig proCfg = new ProjectConfig();
        proCfg.setProjectId(projectId);
        proCfg.setProjectSecret(projectSecret);
        proCfg.setItsmApiUrl(apisUrl);
        ...
        if (0 != result.getErrCode()) {
            LOGGER.error("--项目初始化失败：errCode=" + result.getErrCode() + " msg=" + result.getMsg());
        } else {
            LOGGER.info("--项目初始化成功！errCode=" + result.getErrCode() + " msg=" + result.getMsg());
    ...
    public static PosBean setXYPosBean(String page, int x, int y, int width) {
        PosBean posBean = setXYPosBean(page, x, y);
    ...
    public static FileDigestSignResult platformSignByStreamm(String srcPdfFile) {
        ...
        if (0 != fileDigestSignResult.getErrCode()) {
            LOGGER.info("平台自身PDF摘要签署（文件流）失败，errCode=" + fileDigestSignResult.getErrCode() + " msg=" + fileDigestSignResult.getMsg());
        } else {
     ...
     * SignType.Single);
     */
    public static FileDigestSignResult platformSignByFile(String srcPdfFile, String signedPdfFile, PosBean posBean) {
        // 设置文件签署的PDF文档信息
        ...
        if (0 != fileDigestSignResult.getErrCode()) {
            LOGGER.info("平台自身PDF摘要签署（文件）失败，errCode=" + fileDigestSignResult.getErrCode() + " msg=" + fileDigestSignResult.getMsg());
        } else {
    ...
    public static FileDigestSignResult platformSignByStreamm(byte[] pdfFileStream, PosBean posBean, SignType signType) {
    ...
    public static FileDigestSignResult userPersonSignByStream(byte[] pdfFileStream, String accountId, String sealData) {
        ...
        SignPDFStreamBean signPDFStreamBean = setSignPDFStreamBean(pdfFileStream);
        // 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
        PosBean posBean = setKeyPosBean("甲方：", 0, 0, 90);
        // 设置签署类型为 关键字签章
        SignType signType = SignType.Key;
    ...
    public static FileDigestSignResult userPersonSignByStream(byte[] pdfFileStream, String accountId, String sealData, PosBean posBean, SignType signType) {
        SignPDFStreamBean signPDFStreamBean = setSignPDFStreamBean(pdfFileStream);
        System.out.println("----开始平台个人客户的PDF摘要签署...");
        UserSignService userSignService = UserSignServiceFactory.instance();
        ...
        if (0 != fileDigestSignResult.getErrCode()) {
            LOGGER.info("平台个人客户的PDF摘要签署失败，errCode=" + fileDigestSignResult.getErrCode() + " msg=" + fileDigestSignResult.getMsg());
        } else {
            LOGGER.info("平台个人客户的PDF摘要签署成功！签署记录SignServiceId = " + fileDigestSignResult.getSignServiceId() + ", 请保存该签署记录Id！");
            System.out.println("平台个人客户的PDF摘要签署成功！签署记录SignServiceId = " + fileDigestSignResult.getSignServiceId() + ", 请保存该签署记录Id！");
        }
        return fileDigestSignResult;
    }
     ...
     * signPDFStreamBean, posBean, SignType.Single);
     */
    public static FileDigestSignResult userPersonSignByFile(String srcPdf, String signedPdf, String accountId, String sealData, PosBean posBean) {
        ...
        FileDigestSignResult fileDigestSignResult = userSignService.localSignPDF(accountId, sealData, signPDFFileBean,
                posBean, signType);
        if (0 != fileDigestSignResult.getErrCode()) {
            LOGGER.error("平台个人客户的PDF摘要签署失败，errCode=" + fileDigestSignResult.getErrCode() + " msg=" + fileDigestSignResult.getMsg());
        } else {
    ...
    public static FileDigestSignResult userOrganizeSignByStream(byte[] pdfFileStream, String accountId, String sealData) {
        ...
        FileDigestSignResult fileDigestSignResult = userSignService.localSignPDF(accountId, sealData, signPDFStreamBean,
                posBean, signType);
        if (0 != fileDigestSignResult.getErrCode()) {
            LOGGER.info("平台企业客户的PDF摘要签署失败，errCode=" + fileDigestSignResult.getErrCode() + " msg="
                    + fileDigestSignResult.getMsg());
        } else {
     ...
     */
    public static String addPersonAccount(PersonBean personBean) {
...
//		organizeBean.setMobile("188****0787");
    ...
    public static void deleteAccount(String accountId) {
        ...
        if (0 != result.getErrCode()) {
            LOGGER.info("注销个人账户失败，errCode=" + result.getErrCode() + " msg=" + result.getMsg());
        } else {
    ...
    public static void updatePersonAccount(String accountId, String mobile) {
        ...
        if (0 != result.getErrCode()) {
            LOGGER.info("更新个人账户失败，errCode=" + result.getErrCode() + " msg=" + result.getMsg());
        } else {
    ...
    public static void updateOrganizeAccount(String accountId, String mobile) {
        ...
        if (0 != result.getErrCode()) {
            LOGGER.info("更新企业账户失败，errCode=" + result.getErrCode() + " msg=" + result.getMsg());
        } else {
    ...
    public static boolean saveSignedByStream(byte[] signedStream, String signedFolder, String signedFileName) {
        ...
        if (0 != Integer.parseInt(fileResult.get("errCode"))) {
            LOGGER.info("保存签署后文件失败，errCode=" + fileResult.get("errCode") + " msg=" + fileResult.get("msg"));
        } else {
            isSuccess = true;
