...
CREATE TABLE `saas_order_application` (
  ...
  `late_fee_ratio` decimal(5,4) NOT NULL DEFAULT '0.0000' COMMENT '逾期利率',
  `borrow_purpose` varchar(32) DEFAULT NULL COMMENT '借款意图',
  `repayment_date` date NOT NULL COMMENT '账单应还日',
...
CREATE TABLE `saas_order` (
  ...
  `created_dt` datetime NOT NULL COMMENT '订单申请时间',
  `term_url` varchar(256) DEFAULT NULL COMMENT '借款协议URL地址',
  `relation_order_id` varchar(32) DEFAULT NULL COMMENT '展期关联订单ID',
  `order_status` smallint(4) NOT NULL COMMENT '订单状态',  
  `primary_reviewer_code` varchar(32) DEFAULT NULL COMMENT '初审员',
...
CREATE TABLE `saas_order_bill_detail` (
  ...
  `relation_order_bill_detail_id` varchar(32) DEFAULT NULL COMMENT '展期关联账单ID',
